...
VectorX<bool> MaximumClique(SparseMatrix<bool> adjacency_matrix) {
  ...
  for (int j = 0; j < n; ++j) {
    ...
    int zero_index = 0;
    for (SparseMatrix<bool>::InnerIterator it(adjacency_matrix, j); it;
         ++it) {
...
std::vector<HPolyhedron> IrisRegionsFromCliqueCover(
    ...
    const Eigen::Ref<const Eigen::MatrixXd>& original_points, const IrisOptions& options,
    int minimum_clique_size, bool parallelize) {
