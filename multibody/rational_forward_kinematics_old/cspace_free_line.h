...
//  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(CspaceLineTuple)
 ...
 private:
  ...
  // and ν.
  copyable_unique_ptr<solvers::MathematicalProgram> psatz_variables_and_psd_constraints_;
...
class CspaceFreeLine : public CspaceFreeRegion {
 public:
  ...
  std::unique_ptr<solvers::MathematicalProgram> MakeCertificationProgram(
      const Eigen::Ref<const Eigen::VectorXd>& s0,
      const Eigen::Ref<const Eigen::VectorXd>& s1) const;

  std::vector<std::vector<int>> separating_plane_to_tuples() const {
    return separating_plane_to_tuples_;
  }
