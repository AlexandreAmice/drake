...
CspaceLineTuple::CspaceLineTuple(
  ...
  if (p_.TotalDegree() > 0) {
    ...
    if (p_.TotalDegree() % 2 == 0) {
      auto [nu, Q_nu] = psatz_variables_and_psd_constraints_.get_mutable()->NewSosPolynomial(
...
std::unique_ptr<solvers::MathematicalProgram> CspaceFreeLine::MakeCertificationProgram(
    const Eigen::Ref<const Eigen::VectorXd>& s0,
    const Eigen::Ref<const Eigen::VectorXd>& s1) const {
  std::unique_ptr<solvers::MathematicalProgram> prog{new solvers::MathematicalProgram()};
  prog.get()->AddDecisionVariables(separating_plane_vars_);
  prog.get()->AddIndeterminates(solvers::VectorIndeterminate<1>(mu_));
  for (int i = 0; i < static_cast<int>(separating_planes().size()); ++i) {
    AddCertifySeparatingPlaneConstraintToProg(prog.get(), i, s0, s1);
  }
  return prog;
}
