...
//
//  Eigen::MatrixXd A_new(2 * seed_point.rows() * num_perm_dim * num_rot,
//                        seed_point.rows());
//  Eigen::VectorXd b_new(2 * seed_point.rows() * num_perm_dim * num_rot);
//  A_new.topRows(2 * seed_point.rows()) = unit_box.A();
//  b_new.topRows(2 * seed_point.rows()) = unit_box.b() + seed_point;
//
//  Eigen::MatrixXd A_cur(2 * seed_point.rows(), seed_point.rows());
//  Eigen::VectorXd b_cur(2 * seed_point.rows());
//
//  int cur_row = 2 * seed_point.rows();
//  HPolyhedron* cur_transformed_polytope_ptr = nullptr;
//  for (Eigen::MatrixXd rot : SOnMembers) {
//    for (Eigen::MatrixXd perm : perms) {
//      *cur_transformed_polytope_ptr = SameDimensionalAffineTransform(
...
//                                                      const int n) {
//  std::vector<Eigen::MatrixXd> members(k);
//  Eigen::Matrix2d rot;
