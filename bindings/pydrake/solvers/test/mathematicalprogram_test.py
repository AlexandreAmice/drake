...
class TestMathematicalProgram(unittest.TestCase):
    ...
    def test_sdp(self):
        ...
        self.assertEqual(
            prog.positive_semidefinite_constraints()[1].evaluator().matrix_rows(), 2
        )
    ...
    def test_linear_constraints(self):
        ...
        dense2 = prog.AddLinearEqualityConstraint(Aeq=A_dense, beq=np.zeros(2), vars=x)
        # Ensure that the dense version of the binding has been called.
    ...
    def test_add_linear_matrix_inequality_constraint(self):
        ...
        constraint2 = prog.AddLinearMatrixInequalityConstraint(
            X=np.array(
                [[1, 2 + x[0], 3 - x[0]], [2 + x[0], 1 - x[0], 1], [3 - x[0], 1, 2]]
            )
        )
        self.assertIsInstance(
    ...
    def test_solver_options(self):
        ...
        with catch_drake_warnings(expected_count=1):
            self.assertEqual(dut.get_standalone_reproduction_file_name(), "repro.txt")
        with catch_drake_warnings(expected_count=1):
    ...
    def test_sort_decision_variables(self):
        prog = mp.MathematicalProgram()
        x0 = sym.Variable("x0")
        x1 = sym.Variable("x1")
        y0 = sym.Variable("y0")
        ...
        expected_order = [x0, x1, y0, y1]
        for i, var in enumerate(prog.decision_variables()):
            self.assertEqual(var.get_id(), expected_order[i].get_id())

        new_order = [y0, x1, x0, y1]
        prog.SortDecisionVariables(new_order)
        for i, var in enumerate(prog.decision_variables()):
            self.assertEqual(var.get_id(), new_order[i].get_id())
