...
std::unique_ptr<MathematicalProgram> SolveSDPwithSecondOrderConeExample1(
  ...
  if (solver_is_available) {
    ...
    const auto x_val = result.GetSolution(x);
    EXPECT_NEAR((C0 * X_val).trace() + x_val(0), result.get_optimal_cost(),
                tol);
    EXPECT_NEAR((Eigen::Matrix3d::Identity() * X_val).trace() + x_val(0), 1,
                tol);
...
std::unique_ptr<MathematicalProgram> SolveSDPwithSecondOrderConeExample2(
  ...
  if (solver_is_available) {
    ...
    EXPECT_NEAR(X_val(0, 0) + 2 * X_val(1, 1) + X_val(2, 2) + 3 * x_val, 3,
                tol);
...
std::unique_ptr<MathematicalProgram> TestSDPDualSolution1(
  ...
  if (solver_is_available) {
    ...
    // bounding box constraint on x0 is -.25, and the sensitivity to the
    // bounding box constraint on x2 is -1.
    ...
    // Complementarity condition ensures the inner product of X and its dual is
    // 0.
    ...
    // dual solution to the left hand side of (1) is what Mosek/SCS returns as
    // the dual solution.
