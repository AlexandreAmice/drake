...
// Test that relaxation vars has the expected number of variables and that those
// variables are arranged in the expected order to produce X. Also test the
// presence of the bounding box constraint X(-1,-1) = 1.
void TestRelaxationVars(const MathematicalProgram* prog,
                        const MathematicalProgram* relaxation,
                        const MatrixX<Variable> X) {
  const int original_prog_vars_size = prog->num_vars();
  const int expected_X_rows = original_prog_vars_size + 1;
  const int expected_num_vars = expected_X_rows * (expected_X_rows + 1) / 2;

  EXPECT_EQ(relaxation->num_vars(), expected_num_vars);
  EXPECT_EQ(X.rows(), expected_X_rows);
  // If X = [Y   y]
  //        [yᵀ  1],
  // then relaxation->decision_variables() == [y, flat_lower(Y), 1] with
  // flat_lower(Y) being the columns of the lower triangular part of Y stacked
  // column-wise. We check this layout below.
  int count = original_prog_vars_size;
  for (int r = 0; r < X.rows() - 1; ++r) {
    for (int c = r; c < X.rows() - 1; ++c) {
      EXPECT_TRUE(X(r, c).equal_to(X(c, r)));

      EXPECT_TRUE(X(r, c).equal_to(relaxation->decision_variables()(count)));
      ++count;
    }
    EXPECT_TRUE(X(r, X.rows() - 1).equal_to(X(X.rows() - 1, r)));
    EXPECT_TRUE(
        X(r, X.rows() - 1).equal_to(relaxation->decision_variables()(r)));
  }
  EXPECT_TRUE(
      X(X.rows() - 1, X.rows() - 1)
          .equal_to(relaxation->decision_variables()(expected_num_vars - 1)));
  // Check that X(-1,-1) == 1
  EXPECT_EQ(Variables(relaxation->bounding_box_constraints().at(0).variables()),
            Variables{X(X.rows() - 1, X.rows() - 1)});
  EXPECT_EQ(
      relaxation->bounding_box_constraints().at(0).evaluator()->upper_bound()(
          0),
      1);
  EXPECT_EQ(
      relaxation->bounding_box_constraints().at(0).evaluator()->lower_bound()(
          0),
      1);
  ...
  std::vector<int> indices =
      relaxation->FindDecisionVariableIndices(prog->decision_variables());
  EXPECT_EQ(indices.size(), prog->decision_variables().size());
}
...
GTEST_TEST(MakeSemidefiniteRelaxationTest, NoCostsNorConstraints) {
  ...
  const auto relaxation_and_X = MakeSemidefiniteRelaxationLinearConstraintsAndComputeMinorCliques(
      prog, &computed_sparsity_groups);
  MathematicalProgram* relaxation = relaxation_and_X.first.get();
  MatrixX<Variable> X{relaxation_and_X.second};

  TestRelaxationVars(&prog, relaxation, X);

  // X(-1,-1) = 1 is the only constraint.
  EXPECT_EQ(relaxation->bounding_box_constraints().size(), 1);
  EXPECT_EQ(relaxation->GetAllConstraints().size(), 1);

  // Nothing is coupling the variables so computed_sparsity_groups should be
  // empty
  EXPECT_EQ(computed_sparsity_groups.size(), 0);
}
...
GTEST_TEST(MakeSemidefiniteRelaxationTest, UnsupportedCost) {
  ...
  DRAKE_EXPECT_THROWS_MESSAGE(
      MakeSemidefiniteRelaxationLinearConstraintsAndComputeMinorCliques(prog,
                                                                        &computed_sparsity_groups),
      ".*GenericCost was declared but is not supported.");
  DRAKE_EXPECT_THROWS_MESSAGE(
      MakeSemidefiniteRelaxationLinearConstraintsAndComputeMinorCliques(prog, std::nullopt),
      ".*GenericCost was declared but is not supported.");
}
...
GTEST_TEST(MakeSemidefiniteRelaxationTest, LinearCostWithOffset) {
  MathematicalProgram prog;
  ...
  SetRelaxationInitialGuess(y_test, X, relaxation);
  EXPECT_NEAR(
      relaxation->EvalBindingAtInitialGuess(relaxation->linear_costs()[0])[0],
      a.transpose() * y_test + b, 1e-12);

  // The computed sparsity group should only contain (y(0), y(2), 1) since
  // a(1) = 0.
  EXPECT_EQ(computed_sparsity_groups.size(), 1);
  EXPECT_EQ(*computed_sparsity_groups.begin(), (Variables{y(0), y(2), one}));
}
...
GTEST_TEST(MakeSemidefiniteRelaxationTest, LinearCostAutoSparseNoOffset) {
  ...
  // The computed sparsity group should only contain (y(0), y(2)) since
  // a(1) = 0 and b = 0
  EXPECT_EQ(computed_sparsity_groups.size(), 1);
  EXPECT_EQ(*computed_sparsity_groups.begin(), (Variables{y(0), y(2)}));
}

GTEST_TEST(MakeSemidefiniteRelaxationTest, QuadraticCostWithOffset) {
  MathematicalProgram prog;
  const auto y = prog.NewContinuousVariables<3>("y");
  const Matrix3d Q{{1, 2, 0}, {2, 3, 4}, {0, 4, 5}};
  const Vector3d b{1, 3, 0};
  const double c{0.5};
  prog.AddQuadraticCost(Q, b, c, y);
  ...
  // Quadratic costs are converted to linear costs
  EXPECT_EQ(relaxation->linear_costs().size(), 1);
  EXPECT_EQ(relaxation->GetAllCosts().size(), 1);

  const Vector3d y_test(1.3, 0.24, -2.7);
  double expected_initial_guess_cost{
      (0.5 * y_test.transpose() * Q * y_test + b.transpose() * y_test)(0) + c};
  SetRelaxationInitialGuess(y_test, X, relaxation);
  EXPECT_NEAR(
      relaxation->EvalBindingAtInitialGuess(relaxation->linear_costs()[0])[0],
      expected_initial_guess_cost, 1e-12);

    std::cout << fmt::format("lin cost  a = {}", fmt_eigen(relaxation->linear_costs()[0].evaluator()->a())) << std::endl;
    std::cout << fmt::format("y = {}", fmt_eigen(relaxation->decision_variables())) << std::endl;


  // The computed sparsity group should only contain (y(0), y(1), 1) and
  // (y(1),y(2), 1) since Q(2,0) = 0.
  EXPECT_EQ(computed_sparsity_groups.size(), 2);
  EXPECT_EQ(*computed_sparsity_groups.begin(),
            GetMinorVariablesFromMakeSemidefiniteRelaxationVars(
                relaxation->decision_variables(), prog.num_vars(), {0, 1, 3}));
  EXPECT_EQ(*computed_sparsity_groups.begin(),
            GetMinorVariablesFromMakeSemidefiniteRelaxationVars(
                relaxation->decision_variables(), prog.num_vars(), {1, 2, 3}));
  EXPECT_EQ(*next(computed_sparsity_groups.begin()),
            (Variables{y(1), y(2), one}));
}
