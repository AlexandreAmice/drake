 ...
 * [range_start, range_end), optionally using generated initial guesses,
 * solver options, and solver ids.
 *
 * Callback return values follow these conventions:
 * - program generator may return nullptr to skip an index;
 * - initial guess and solver options generators may return nullptr;
 * - solver id generator may return nullopt to choose the best available solver.
 *
 * The callback functions may be invoked concurrently from multiple threads.
 * Program pointers returned by @p prog_generator must remain valid until this
 * method returns.
 *
...
std::vector<MathematicalProgramResult> SolveInParallel(
    const SolveInParallelProgramGenerator& prog_generator, int64_t range_start,
    int64_t range_end,
