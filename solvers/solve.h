...
using SolveInParallelProgramGenerator =
    std::function<const MathematicalProgram*(int thread_num, int64_t i)>;
using SolveInParallelInitialGuessGenerator =
    std::function<const Eigen::VectorXd*(int thread_num, int64_t i)>;
using SolveInParallelSolverOptionsGenerator =
    std::function<const SolverOptions*(int thread_num, int64_t i)>;
using SolveInParallelSolverIdGenerator =
    std::function<std::optional<SolverId>(int thread_num, int64_t i)>;

/** The i'th solve inputs to be filled by a SolveInParallel callback. */
struct SolveInParallelIthProgramData {
  const MathematicalProgram* prog{};
...
/** Callback that fills the i'th solve inputs.
 * Returns true to proceed with solving index @p i, or false to skip it.
 */
using SolveInParallelIthProgramGenerator =
    std::function<bool(int thread_num, int64_t i,
        SolveInParallelIthProgramData* ith_program_data)>;

/**
 * Solves the programs generated by @p prog_generator for all indices in
 * [range_start, range_end), where all i'th solve inputs are generated via a
 * single callback.
 *
 * Callback conventions:
 * - returns false to skip index i (result[i] remains unset);
 * - returns true and sets `ith_program_data->prog` to nullptr to also skip i;
 * - when `ith_program_data->solver_id` is nullopt, the best solver is chosen.
 *
 * The callback functions may be invoked concurrently from multiple threads.
 * Program pointers returned by @p make_ith_program must remain valid until this
 * method returns.
 *
 * @throws std::exception if range_start > range_end.
 *
 * @throws std::exception if any generated program cannot be solved.
 */
...
/**
 * Convenience overload that generates i'th solve inputs using separate
 * callbacks for program / initial guess / options / solver id.
 */
std::vector<MathematicalProgramResult> SolveInParallel(
    const SolveInParallelProgramGenerator& prog_generator,
    int64_t range_start, int64_t range_end,
    const std::optional<SolveInParallelInitialGuessGenerator>&
        initial_guess_generator = std::nullopt,
    const std::optional<SolveInParallelSolverOptionsGenerator>&
        solver_options_generator = std::nullopt,
